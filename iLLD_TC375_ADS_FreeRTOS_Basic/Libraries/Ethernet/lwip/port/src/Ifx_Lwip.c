...
typedef struct
{
    ...
    ip_addr_t  gateway;
    sys_sem_t  readySem;
} Ifx_Lwip_InitContext;

static void Ifx_Lwip_tcpip_init_done(void *arg);
#endif

static void Ifx_Lwip_handleLinkStatus(void);
...
void Ifx_Lwip_onTimerTick(void)
{
    /* Timer management handled by lwIP tcpip thread in RTOS mode */
}

void Ifx_Lwip_pollTimerFlags(void)
{
    /* Nothing to do in RTOS mode */
}
...
{
    /* Fetch pending frames from the MAC and hand them to lwIP */
    ifx_netif_input(&g_Lwip.netif);

#if NO_SYS == 0
    static uint32_t s_linkPollDivider = 0U;
    if (++s_linkPollDivider >= 30U)
    {
        Ifx_Lwip_handleLinkStatus();
        s_linkPollDivider = 0U;
    }
#endif
}
...
static void Ifx_Lwip_tcpip_init_done(void *arg)
{
    Ifx_Lwip_InitContext *ctx = (Ifx_Lwip_InitContext *)arg;

    g_Lwip.eth_addr = ctx->ethAddr;

    netif_add(&g_Lwip.netif,
              &ctx->ipaddr,
              &ctx->netmask,
              &ctx->gateway,
              (void *)0,
...
//    initUART();
    ...
    Ifx_Lwip_InitContext ctx;
    memset(&ctx, 0, sizeof(ctx));
    memcpy(&ctx.ethAddr, &ethAddr, sizeof(ethAddr));
    ctx.ipaddr = default_ipaddr;
    ctx.netmask = default_netmask;
    ctx.gateway = default_gw;

    LWIP_ASSERT("sys_sem_new", sys_sem_new(&ctx.readySem, 0) == ERR_OK);
    tcpip_init(Ifx_Lwip_tcpip_init_done, &ctx);
    sys_arch_sem_wait(&ctx.readySem, 0);
    sys_sem_free(&ctx.readySem);
