...
void SOMEIP_Callback(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, uint16 port)
{
    ...
    if (p != NULL)
    {
		...
		else
		{
			...
			if (MessageType == 0x00 && MethodID == APP_COMM_SOMEIP_METHOD_DRIVE_COMMAND)
			{
				uint16 payload_len = (p->len > 16U) ? (uint16)(p->len - 16U) : 0U;
				const uint8 *payload = &rxBuf[16];
				...
				if (txbuf != NULL) {
					...
					if (err == ERR_OK) {
						my_printf("Drive command response sent\n");
					} else {
						my_printf("Drive command response failed\n");
					}
					udp_disconnect(upcb);
					pbuf_free(txbuf);
				} else {
					my_printf("Failed to allocate memory for SOME/IP response\n");
				}
			}
			else
			{
