...
#if LWIP_IGMP
static err_t tc375_igmp_mac_filter(struct netif *n, const ip4_addr_t *group, u8_t action);
#endif
...
/*-----------------------------------Macros-----------------------------------*/
/******************************************************************************/
// 기존에 있는 매크로 바로 아래에 추가
#define IFX_LWIP_IGMP_PERIOD        (100U / IFX_LWIP_TIMER_TICK_MS)  /* 100 ms */
#define IFX_LWIP_FLAG_IGMP          (1U << 7)

#if LWIP_IGMP
static uint16 s_igmp_tick = 0;
#endif
...
void Ifx_Lwip_onTimerTick(void)
{
    ...
    //added
#if LWIP_IGMP
    /* --- IGMP 주기: 로컬 카운터로 100ms마다 플래그 --- */
    s_igmp_tick += 1;
    if (s_igmp_tick >= IFX_LWIP_IGMP_PERIOD) {
        s_igmp_tick = 0;
        timerFlags |= IFX_LWIP_FLAG_IGMP;
    }
#endif
...
void Ifx_Lwip_pollTimerFlags(void)
{
    ...
    //added
#if LWIP_IGMP
    if (timerFlags & IFX_LWIP_FLAG_IGMP) {
        igmp_tmr();    /* ★ IGMP state machine을 100ms마다 돌림 */
    }
#endif
...
static err_t tc375_igmp_mac_filter(struct netif *n, const ip4_addr_t *group, u8_t action)
{
    // (선택) 실제 하드웨어 멀티캐스트 필터를 설정하려면 여기에서
    // group IP(224.x.x.x) -> MAC(01:00:5E:xx:xx:xx) 계산 후 IfxGeth API로 add/del 하면 됨.
    // 지금은 스택 진행만 보장하는 no-op로 충분.
    (void)n; (void)group; (void)action;
    return ERR_OK;
}
#endif
...
//    initUART();
    ...
    IP4_ADDR(&default_ipaddr, 10,167,152,20);
    ...
    struct netif *nif = netif_add(&g_Lwip.netif, &default_ipaddr, &default_netmask, &default_gw,
        (void *)0, ifx_netif_init, ethernet_input);

    if (nif == NULL) {
        my_printf("netif_add failed\n");  // void 함수면 값을 반환하지 말 것
        return;                            // 'return false;' 금지
    }
...
/*
#if LWIP_IGMP
  #ifdef netif_set_igmp_mac_filter
    netif_set_igmp_mac_filter(&g_Lwip.netif, tc375_igmp_mac_filter);
  #endif
  g_Lwip.netif.flags |= NETIF_FLAG_IGMP;
#endif
*/

//    g_IfxGeth.gethSFR->MAC_PACKET_FILTER.B.PR = 1; // Promiscuous (모든 프레임 수신)

    netif_set_default(&g_Lwip.netif);
    netif_set_link_up(&g_Lwip.netif); // added
