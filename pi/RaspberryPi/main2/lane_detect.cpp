...
vector<Point> sliding_lane_points(const Mat& binary_img, bool is_left, int nwindows, int margin, int minpix) {
    ...
    int base;
    if (is_left) {
    int left_end = static_cast<int>(w * 0.40);
    minMaxLoc(hist(Range::all(), Range(0, left_end)), &minVal, &maxVal, &minLoc, &maxLoc);
    base = maxLoc.x;
} else {
    int right_start = static_cast<int>(w * 0.60);
    minMaxLoc(hist(Range::all(), Range(right_start, w)), &minVal, &maxVal, &minLoc, &maxLoc);
    base = maxLoc.x + right_start;
}
